...
*/
/// The printing function printq
void printq(Queue *pq)
{
    Node *print;
    print = pq -> front;

    if(pq -> items == 0) // if is empty
        return; // return and continua
// other wise it goes in an infinite loop

    while(print != NULL)
    {
        printf("%d ", print->item); // printing the item
        print = print -> next; // taking the next item
    }
}
